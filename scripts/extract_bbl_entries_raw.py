...
def main():
    if len(sys.argv) < 2:
        print("Usage: python extract_bbl_entries_raw.py <bbl_file> [output_csv]")
        sys.exit(1)

    bbl_path = Path(sys.argv[1])
    output_path = Path(sys.argv[2]) if len(sys.argv) > 2 else bbl_path.parent / "bbl_entries_raw.csv"
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['citation_key', 'formatted_entry'])
        writer.writeheader()
    ...
    for i, entry in enumerate(entries[:3]):
        ...
        preview = entry['formatted_entry'][:200] + "..." if len(entry['formatted_entry']) > 200 else entry['formatted_entry']
        print(f"    {preview}")
