...
def check_author_name(text: str) -> List[str]:
    ...
    org_indicators = [
        'OECD', 'UN', 'WHO', 'UNESCO', 'UNECE', 'EPA',
        'European Commission', 'European Parliament',
        'BBC', 'Bloomberg', 'Fashion United', 'Fashion Revolution',
        'H&M Foundation', 'WBCSD', 'GS1', 'ITC',
        'Anthropic', 'Google', 'OpenAI', 'Microsoft',
        'Web page', 'Website', 'Blog post', 'News article'
    ]

    for indicator in org_indicators:
        if indicator.lower() in text.lower():
            issues.append(f"Author field contains organization name: '{indicator}'")
            break
...
def main():
    ...
    for line_num, full_match, text, url in citations:
        ...
        if is_academic:
            ...
            if any(domain in url.lower() for domain in ['blog', 'medium.com', 'news', 'article']):
    ...
    print(f"Non-academic links (potential footnotes): {len(non_academic_links)}")
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"**Generated:** 2025-10-31\n")
        ...
        if broken_urls:
            ...
            for item in broken_urls:
                ...
                f.write("**Recommendation:** Verify URL and update if necessary\n\n")
                f.write("---\n\n")
        ...
        if non_academic_links:
            ...
            for item in non_academic_links:
                f.write(f"### Line {item['line']}\n\n")
                f.write(f"**Citation:** `{item['citation']}`\n\n")
                f.write(f"**Recommendation:** Consider converting to footnote\n\n")
                f.write("---\n\n")
