...
def parse_bbl_file(bbl_path: Path) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        year = ''
        label_match = re.search(r'\((\d{4})\)', key) or re.search(r'\((\d{4})\)', body)
        if label_match:
...
def verify_bbl_quality(bbl_path: Path, strict: bool = False) -> Dict[str, Any]:
    ...
    # Count issues
    hard_failures = sum(len(report['issues'][k]) for k in [
        'domain_titles', 'stub_titles', 'missing_titles', 'temp_keys'
    ])

    soft_failures = sum(len(report['issues'][k]) for k in [
        'malformed_orgs', 'generic_titles'
    ])
...
def print_report(report: Dict[str, Any]):
    ...
    if report['success']:
        ...
        print("   These are warnings - conversion may proceed but quality is suboptimal")
    print("="*70)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify .bbl file quality"
    )
