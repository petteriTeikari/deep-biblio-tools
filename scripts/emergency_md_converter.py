...
def match_url_to_entry(url: str, doi_index, url_index, arxiv_index, all_entries) -> dict:
...
def convert_markdown_citations(md_path: Path, output_path: Path, doi_index, url_index, arxiv_index, all_entries) -> Tuple[int, int, List[str]]:
    ...
    def replace_citation(match):
        ...
        entry = match_url_to_entry(url, doi_index, url_index, arxiv_index, all_entries)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: emergency_md_converter.py <input.md> <database.json> [output_dir]")
        sys.exit(1)
    ...
    all_entries, doi_index, url_index, arxiv_index = load_json_database(json_path)
