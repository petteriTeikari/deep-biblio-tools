...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python diagnose_bib_issues.py <analysis_csv> <references_bib>"
        )
        sys.exit(1)
    ...
    for entry in problematic_entries:
        ...
        if citation_key in bib_entries:
            ...
            print(
                f"   WARNING: Citation key '{citation_key}' not found in references.bib"
            )
    ...
    for d in diagnoses[:5]:  # Show first 5
        if "MISSING_TITLE_IN_BIB" in d["root_causes"]:
            print(
                f"   {d['citation_key']}: Missing title (DOI: {d['doi_value']})"
            )
