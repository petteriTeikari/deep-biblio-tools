...
This version accepts ALL BibTeX entry types including @report, @online, @software, etc.
"""
...
class CitationMismatchAnalyzer:
    ...
    def extract_bib_keys(self) -> set[str]:
        """Extract all entry keys from BibTeX file, including non-standard types."""
        print(f"Extracting bibliography keys from {self.bib_file}...")

        # Create parser that accepts all entry types
        parser = BibTexParser()
        parser.ignore_nonstandard_types = False  # Accept ALL entry types
    ...
    def generate_report(self):
        ...
        report.append(
            "- Accepting ALL BibTeX entry types (@article, @book, @report, @online, @software, etc.)"
        )
                ...
                if missing_key in mappings and mappings[missing_key]:
                    ...
                    for suggested_key, score, reason in mappings[missing_key]:
                        ...
                        if len(title) > 60:
                            title = title[:57] + "..."
                        year = (
                            entry.get("year", entry.get("date", ""))[:4]
                            if entry.get("year") or entry.get("date")
                            else ""
                        )

                        report.append(
                            f"  - `{suggested_key}` [@{entry_type}] (score: {score:.2f}, {reason})"
                        )
        ...
        if unused_entries:
            ...
            for key in unused_entries:
                ...
                year = (
                    entry.get("year", entry.get("date", "Unknown"))[:4]
                    if entry.get("year") or entry.get("date")
                    else "Unknown"
                )
            ...
            for year in sorted(by_year.keys(), reverse=True):
                ...
                for key, entry in by_year[year]:
                    ...
                    if len(title) > 70:
                        title = title[:67] + "..."
                    report.append(f"- `{key}` [@{entry_type}]: {author}")
                    report.append(f'  "{title}"')
                report.append("")
        else:
        ...
        if missing_citations:
            ...
            report.append(
                "- Most likely cause: Better BibTeX key format changes or typos"
            )
            report.append(
...
def main():
    ...
    # Save report
    report_file = tex_file.parent / "citation_mismatch_report_all_types.md"
    ...
    if missing:
        ...
        print("\nMissing citations:")
        for citation in missing[:10]:  # Show first 10
            print(f"  - {citation}")
        if len(missing) > 10:
    ...
    if unused:
        print(f"\nFound {len(unused)} unused bibliography entries")
