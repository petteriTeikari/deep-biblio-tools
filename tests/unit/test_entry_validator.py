...
class TestValidateTitle:
    ...
    def test_truncated_title_added_from_url(self, validator):
        ...
        assert any(
            "CRITICAL" in i and "Added from URL" in i
            for i in issues
    ...
    def test_truncated_title_implementation_plan(self, validator):
        ...
        assert any(
            "CRITICAL" in i and "Implementation plan" in i
            for i in issues
        )
...
class TestValidateCreators:
    ...
    def test_missing_creators_warning(self, validator):
        ...
        assert is_valid
        assert any("WARNING" in i and ("author" in i.lower() or "creator" in i.lower()) for i in issues)
    ...
    def test_valid_creators(self, validator):
        ...
        creator_issues = [i for i in issues if "creator" in i.lower() or "author" in i.lower()]
    ...
    def test_creator_missing_lastname(self, validator):
        ...
        metadata = {
            "title": "Article Title",
            "creators": [
                {"creatorType": "author", "firstName": "John"}  # Missing lastName
            ],
            "date": "2023"
        }
        is_valid, issues = validator.validate(metadata)
...
class TestValidateDate:
    ...
    def test_valid_year(self, validator):
        ...
        date_issues = [i for i in issues if "year" in i.lower() or "date" in i.lower()]
    ...
    def test_extract_year_various_formats(self, validator):
        ...
        for date_str, expected_year in test_cases:
            ...
            assert year == expected_year, f"Failed for '{date_str}': expected {expected_year}, got {year}"
...
class TestItemTypeSpecificValidation:
    ...
    def test_journal_article_without_publication(self, validator):
        ...
        assert is_valid
        assert any("WARNING" in i and "publication" in i.lower() for i in issues)
    ...
    def test_conference_paper_without_proceedings(self, validator):
        ...
        assert is_valid
        assert any("WARNING" in i and "proceedings" in i.lower() for i in issues)
