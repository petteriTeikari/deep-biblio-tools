...
from src.converters.md_to_latex.citation_manager import Citation, CitationManager
...
def test_validate_doi_success(citation_manager):
    ...
    with patch("requests.head") as mock_head:
        ...
        assert "10.48550/arXiv.2506.17419" in citation_manager._doi_validation_cache
...
def test_validate_doi_failure_404(citation_manager):
    ...
    with patch("requests.head") as mock_head:
        ...
        assert citation_manager._doi_validation_cache["10.1016/j.compind.2021.107130"] is False
...
def test_extract_doi_from_doi_url(citation_manager):
    ...
    doi = citation_manager._extract_doi_from_url("https://doi.org/10.1234/example")
    assert doi == "10.1234/example"


def test_extract_doi_from_dx_doi_url(citation_manager):
    """Test extracting DOI from dx.doi.org URL."""
    doi = citation_manager._extract_doi_from_url("https://dx.doi.org/10.1234/example")
    assert doi == "10.1234/example"


def test_extract_doi_from_arxiv_url(citation_manager):
    """Test extracting DOI from arXiv URL."""
    doi = citation_manager._extract_doi_from_url("https://doi.org/10.48550/arXiv.2506.17419")
...
def test_handle_missing_citation_valid_doi_auto_adds(citation_manager, sample_citation_valid):
    ...
    with patch("requests.head") as mock_head, \
         ...
         patch.object(citation_manager, "_fetch_newly_added_entry") as mock_fetch_new:
        ...
        # Assertions
        assert key == "ZOT123"
        mock_matcher.add_to_zotero_library.assert_called_once_with(sample_citation_valid.url)
        # Should not have any CRITICAL errors
        critical_errors = [e for e in citation_manager._citation_errors if e["severity"] == "CRITICAL"]
        assert len(critical_errors) == 0


def test_handle_missing_citation_invalid_doi_returns_temp_key(citation_manager, sample_citation_invalid_doi):
    ...
    with patch("requests.head") as mock_head:
        ...
        key = citation_manager._handle_missing_citation(
            ...
            sample_citation_invalid_doi.url
        )
...
def test_handle_missing_citation_no_doi_returns_temp_key(citation_manager, sample_citation_no_doi):
    ...
    key = citation_manager._handle_missing_citation(
        ...
        sample_citation_no_doi.url
    )

    # Assertions
    assert "Temp" in key
    # Should have WARNING logged
    warnings = [e for e in citation_manager._citation_errors if e["severity"] == "WARNING"]
    assert len(warnings) == 1
    assert warnings[0]["issue"] == "NO_DOI_OR_FAILED_ADD"


def test_handle_missing_citation_incomplete_metadata(citation_manager, sample_citation_valid):
    ...
    with patch("requests.head") as mock_head, \
        ...
        errors = [e for e in citation_manager._citation_errors if e["severity"] == "ERROR"]
        assert len(errors) == 1
        assert errors[0]["issue"] == "INCOMPLETE_METADATA"


def test_handle_missing_citation_zotero_add_fails(citation_manager, sample_citation_valid):
...
def test_policy_enforcement_raises_for_temp_key_with_valid_doi(citation_manager, sample_citation_invalid_doi):
    ...
    with patch("requests.head") as mock_head:
        ...
        with pytest.raises(RuntimeError) as exc_info:
            citation_manager._enforce_no_temp_key_for_valid_doi(sample_citation_invalid_doi)

        assert "Policy violation" in str(exc_info.value)
        assert "agrawalTemp2021" in str(exc_info.value)


def test_policy_enforcement_allows_temp_key_with_invalid_doi(citation_manager, sample_citation_invalid_doi):
    ...
    with patch("requests.head") as mock_head:
        ...
        try:
            ...
            pytest.fail("Policy enforcement should allow Temp key for invalid DOI")


def test_policy_enforcement_allows_temp_key_without_doi(citation_manager, sample_citation_no_doi):
    ...
    try:
        citation_manager._enforce_no_temp_key_for_valid_doi(sample_citation_no_doi)
    except RuntimeError:
        pytest.fail("Policy enforcement should allow Temp key for citations without DOI")


def test_policy_enforcement_allows_non_temp_keys(citation_manager, sample_citation_valid):
    ...
    # Should NOT raise
    try:
        citation_manager._enforce_no_temp_key_for_valid_doi(sample_citation_valid)
    except RuntimeError:
...
def test_error_reporting_groups_by_severity(citation_manager):
    ...
    citation_manager._citation_errors.extend([
        {"severity": "CRITICAL", "issue": "INVALID_DOI", "doi": "10.1234/invalid"},
        {"severity": "ERROR", "issue": "INCOMPLETE_METADATA", "doi": "10.5678/incomplete"},
        {"severity": "WARNING", "issue": "NO_DOI", "url": "https://example.com"},
    ])
...
def test_generate_temp_key_handles_duplicates(citation_manager, sample_citation_valid):
    ...
    assert key1 != key2
    assert key2.startswith(key1.rstrip("a"))  # Should have 'a' suffix or similar
